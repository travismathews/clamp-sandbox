...
export default function ClampPreview() {
  ...
  const getExplanation = (config: ClampConfig) => {
    ...
    return `This element is ${Math.round(clamped)}px at a viewport width of ${viewportWidth}px. It will scale between ${Math.round(minPx)}px and ${Math.round(maxPx)}px.`;
  };
  ...
  return (
    <section className={styles.container}>
      ...
      <fieldset>
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => handleChange("remBoost", parseFloat(e.target.value))}
          />
        </label>
        <label>
      ...
      <p className={styles.paragraph} style={{ fontSize: getClamp(configs.p) }}>
        {getExplanation(configs.h1)}<br />
        {getExplanation(configs.p)}
      </p>
